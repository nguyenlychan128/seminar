...
describe('database — workout-service', () => {
  let connectDatabase;
  ...
  describe('connectDatabase()', () => {
    ...
    it('should pass MONGO_URI env variable to mongoose.connect', async () => {
      process.env.MONGO_URI = 'mongodb://testhost:27017/fitgainer-workout';
      jest.resetModules();
      jest.mock('mongoose');
      ...
      expect(mockMongoose.connect).toHaveBeenCalledWith(
        'mongodb://testhost:27017/fitgainer-workout',
        expect.any(Object)
      );
      delete process.env.MONGO_URI;
    });

    it('should use fallback URI "mongodb://localhost:27017/fitgainer-workout" when MONGO_URI is not set', async () => {
      delete process.env.MONGO_URI;
      jest.resetModules();
      jest.mock('mongoose');
      jest.mock('../../src/utils/logger', () => ({ info: jest.fn(), error: jest.fn(), warn: jest.fn(), debug: jest.fn() }));
      ...
      expect(mockMongoose.connect).toHaveBeenCalledWith(
        'mongodb://localhost:27017/fitgainer-workout',
        expect.any(Object)
      );
    });
    ...
    it('should NOT log raw credentials from MONGO_URI', async () => {
      process.env.MONGO_URI = 'mongodb://admin:secret@prodhost:27017/fitgainer-workout';
      jest.resetModules();
      jest.mock('mongoose');
      jest.mock('../../src/utils/logger', () => ({ info: jest.fn(), error: jest.fn(), warn: jest.fn(), debug: jest.fn() }));
      const { connectDatabase: connect } = require('../../src/config/database');
