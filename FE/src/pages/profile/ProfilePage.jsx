...
export default function ProfilePage() {
  ...
  const { profile, isLoading, hasProfile, updateProfile } = useUserProfile();
  ...
  // Init bmiResult from store on first load (ref guard prevents re-init after update)
  const initializedRef = useRef(false);
  useEffect(() => {
    if (profile && !initializedRef.current) {
      initializedRef.current = true;
      setBmiResult(profile);
    }
  }, [profile]);
  ...
  const displayedProfile = bmiResult ?? profile;
    ...
    <div className="min-h-screen bg-slate-900 px-4 py-12 sm:px-6">
      ...
      {showToast && (
        <div
          role="status"
          aria-live="polite"
          className="fixed right-5 top-5 z-50 flex items-center gap-3 rounded-xl border border-emerald-500/30 bg-emerald-500/15 px-5 py-3 text-sm font-medium text-emerald-400 shadow-lg backdrop-blur-sm"
        >
      ...
      <div className="relative mx-auto max-w-lg">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-base text-slate-400">
            View and update your body information
          </p>
        </div>

        <div className="mb-6">
          <BmiResultCard
            bmi={displayedProfile?.bmi ?? null}
            bodyClassification={displayedProfile?.bodyClassification ?? null}
          />
        </div>
        ...
        <div className="rounded-2xl border border-slate-700/50 bg-slate-800/60 p-6 shadow-xl backdrop-blur-sm sm:p-8">
          ...
          <ProfileForm
            initialValues={{
              height: displayedProfile?.height ?? '',
              weight: displayedProfile?.weight ?? '',
