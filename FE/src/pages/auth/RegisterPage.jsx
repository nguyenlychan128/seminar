...
const REGISTER_ERROR_MESSAGES = {
  DUPLICATE_EMAIL: 'Email is already registered. Try logging in.',
  INVALID_DATA: 'Please check your details and try again.',
  REGISTER_ERROR: 'Registration failed. Please try again.',
};

const STRENGTH_LABELS = ['', 'Very Weak', 'Weak', 'Fair', 'Good', 'Strong'];
const STRENGTH_COLORS = ['', '#ff3b30', '#ff9500', '#ffcc00', '#34c759', '#00c896'];
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const normalizedError = REGISTER_ERROR_MESSAGES[err.code] || err.message || 'Registration failed. Please try again.';
  ...
  const inputClass = (hasError) =>
    `px-4 py-3 text-base bg-slate-700 text-slate-100 border rounded-lg min-h-[44px] transition-all duration-150 placeholder:text-slate-500 focus:outline-none disabled:opacity-60 disabled:cursor-not-allowed ${
      hasError
        ? 'border-red-500 bg-red-500/5 focus:shadow-[0_0_0_3px_rgba(239,68,68,0.1)]'
        : 'border-slate-600 focus:border-emerald-500 focus:shadow-[0_0_0_3px_rgba(16,185,129,0.1)]'
    }`;
      ...
      <div className="relative z-10 w-full max-w-[480px] px-6 py-8">
        ...
        <div className="bg-slate-800 border border-slate-700/50 rounded-xl p-12 shadow-[0_20px_25px_rgba(0,0,0,0.2)] backdrop-blur-sm animate-fade-in-up [animation-delay:0.2s]">
          <div className="text-center mb-12">
            <h1 className="text-3xl font-bold text-slate-100 mb-2 tracking-tight">Create Your Account</h1>
            <p className="text-sm text-slate-400 leading-relaxed">Start your fitness journey today</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6 mb-8">
            <div className="flex flex-col gap-2">
              ...
              />
              {fieldErrors.email && (
                <p className="text-sm text-red-400 m-0">{fieldErrors.email}</p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              {fieldErrors.password && (
                <p className="text-sm text-red-400 m-0">{fieldErrors.password}</p>
              )}
              {password && (
                <div className="mt-2" aria-label={`Password strength: ${STRENGTH_LABELS[strength]}`}>
                  <div className="flex justify-between items-center text-xs text-slate-400 mb-2 uppercase tracking-wide">
                    <span>Password strength</span>
                    <span className="font-semibold transition-colors duration-150" style={{ color: STRENGTH_COLORS[strength] }}>
            ...
            <div className="flex flex-col gap-2">
              ...
              {fieldErrors.confirmPassword && (
                <p className="text-sm text-red-400 m-0">{fieldErrors.confirmPassword}</p>
              )}
            </div>
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-6 py-3.5 min-h-[48px] text-sm font-bold uppercase tracking-wide text-white bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-lg border-0 cursor-pointer transition-all duration-150 shadow-md mt-4 hover:enabled:from-emerald-600 hover:enabled:to-emerald-700 hover:enabled:-translate-y-0.5 hover:enabled:shadow-lg active:enabled:translate-y-0 disabled:opacity-80 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
          ...
          <div className="text-center border-t border-slate-700 pt-6">
            <p className="text-sm text-slate-400 leading-relaxed">
              ...
              <Link to="/login" className="text-emerald-400 font-semibold no-underline hover:text-emerald-300 hover:underline transition-colors duration-150">
                Login here
              </Link>
            </p>
          </div>
